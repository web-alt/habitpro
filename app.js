...
// ── AUTH FLOW ───────────────────────────────────────────────
// Minimum time splash stays visible regardless of how fast loading finishes.
...
async function initApp() {
  ...
  // Wait out any remaining minimum duration before hiding splash
  const elapsed = Date.now() - splashStart;
  const remaining = Math.max(0, SPLASH_MIN_MS - elapsed);
  setTimeout(() => _hideSplash(), remaining);
}
